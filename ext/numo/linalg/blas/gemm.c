...
#define DEF_LINALG_FUNC(tDType, tNArrType, fBlasFnc)                                                     \
  static VALUE _linalg_blas_##fBlasFnc(int argc, VALUE* argv, VALUE self) {                              \
    ...
    if (CLASS_OF(b) != tNArrType) {                                                                      \
      b = rb_funcall(tNArrType, rb_intern("cast"), 1, b);                                                \
    }                                                                                                    \
    if (!RTEST(nary_check_contiguous(b))) {                                                              \
      b = nary_dup(b);                                                                                   \
    }                                                                                                    \
    if (!NIL_P(c)) {                                                                                     \
      if (CLASS_OF(c) != tNArrType) {                                                                    \
        c = rb_funcall(tNArrType, rb_intern("cast"), 1, c);                                              \
      }                                                                                                  \
    ...
    if (!NIL_P(c)) {                                                                                     \
      ...
      ndfunc_arg_in_t ain[3] = { { tNArrType, 2 }, { tNArrType, 2 }, { OVERWRITE, 2 } };                 \
      ndfunc_t ndf = { _iter_##fBlasFnc, NO_LOOP, 3, 0, ain, aout };                                     \
      na_ndloop3(&ndf, &opt, 3, a, b, c);                                                                \
      ret = c;                                                                                           \
    } else {                                                                                             \
      c = INT2NUM(0);                                                                                    \
      ndfunc_arg_in_t ain[3] = { { tNArrType, 2 }, { tNArrType, 2 }, { sym_init, 0 } };                  \
