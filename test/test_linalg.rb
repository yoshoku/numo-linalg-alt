...
class TestLinalg < Minitest::Test # rubocop:disable Metrics/ClassLength
  ...
  def test_tanhm
    a = Numo::DFloat[[1, 2], [-2, 1]]
    a_tanhm = Numo::Linalg.tanhm(a)
    expected = -1.0i * Numo::Linalg.tanm(1.0i * a)
    error = (expected - a_tanhm).abs.max

    assert_operator(error, :<, 1e-7)
    assert_kind_of(Numo::DFloat, a_tanhm)
    ...
    assert_operator(error, :<, 1e-7)
    assert_kind_of(Numo::DComplex, a_tanhm)
  end

  def test_sqrtm
    a = Numo::DFloat[[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    b = Numo::Linalg.sqrtm(a)
    error = (a - b.dot(b)).abs.max
